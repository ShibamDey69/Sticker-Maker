...
const ToWebp = async (
    ...
    try {
        ...
        inputPath = mimeType?.includes('video') || mimeType?.includes('webp') ? await toGif(inputPath) : inputPath
            ...
            metaInfo.background
                ? `-filter_complex "color=${metaInfo.background} [c]; [c][0]scale2ref[cs][0s];[cs][0s]overlay=shortest=1, unsharp=3:3:0.5${
                      ...
                      metaInfo.type === StickerTypes.FIT
                          ? `, ${metaInfo.type.replaceAll(/-vf/g, '').replaceAll(/"/g, '').replace('-filter_complex', '').trim()}`
                          : ''
                  }"`
                : '-pix_fmt yuva420p',
